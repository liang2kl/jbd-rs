...
use common::{
    create_handle, create_journal, existing_journal, mock::write_random_block, sal::UserSystem, setup_logger,
    JOURNAL_SIZE,
};
use jbd_rs::Journal;
...
fn test_revoke() {
    ...
    for i in 0..blocks_dist.len() {
        ...
        for j in 0..blocks.len() {
            ...
            system.block_device().read_block(block_id, &mut disk_data[..]);
            assert_eq!(
...
fn do_one_transaction(system: &Arc<UserSystem>, journal: Arc<RefCell<Journal>>, blocks: Vec<TestInfo>) -> Vec<Vec<u8>> {
